...
export default function TreinoDetalhado({ route }) {
    ...
    // Alterna o check de um exercício
    ...
    // Todos exercícios estão marcados
    const allChecked =
        Object.keys(checks).length === treino.exercicios.length &&
        Object.values(checks).every((checked) => checked);

    const renderExercicio = (ex, index) => (
        <View key={index} className="border border-gray-700 rounded-xl p-5 mt-2">
            ...
            <View className="flex-row justify-between pt-2">
                ...
                <View>
                    ...
                    <Text className="text-colorLight300 text-base">
            ...
            >
                ...
                >
                    <View className="px-5">
                        ...
                        {treino.exercicios.map(renderExercicio)}

                        <View className="px-20 py-5">
                            <TouchableOpacity
                                disabled={!allChecked || loading}
                                onPress={concluirTreino}
                                className={`mt-6 p-3 rounded-full ${!allChecked ? 'bg-gray-500' : 'bg-colorViolet'
                                    }`}
                            >
                                {loading ? (
